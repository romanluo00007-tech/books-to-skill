...
export default function App() {
  ...
  const T = { bg: "#faf8f5", card: "#fff", border: "#e7e5e4", borderHover: "#d6d3d1", text: "#1c1917", textMuted: "#57534e", textDim: "#78716c", textFaint: "#a8a29e", codeBg: "#f5f5f4", accent: "#f0c040", modalOverlay: "rgba(0,0,0,0.4)" };
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"28px 20px 60px"}}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div onClick={() => { setView("home"); setScene(null); }} style={{ cursor: "pointer" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
              ...
              <span style={{ ...s.tag, background: T.accent + "18", color: T.accent, border: `1px solid ${T.accent}40` }}>营销方法论</span>
            </div>
            <div style={{ fontSize: 12, color: T.textDim }}>9 个经典框架 · 9 本畅销书 · 让 AI 拥有顾问级营销能力</div>
          </div>
          <a href="/" style={{ fontSize: 12, color: T.accent, textDecoration: "none", marginTop: 6, display: "inline-block", fontWeight: 500 }}>← 返回门户</a>
        </div>
        ...
        {view === "home" && <>
          ...
          <div style={{ textAlign: "center", padding: "32px 0 36px", marginBottom: 8 }}>
            <div style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: 28, fontWeight: 400, color: T.text, letterSpacing: "-0.02em", marginBottom: 14 }}>营销知识库 Skill</div>
            <div style={{ fontSize: 17, fontWeight: 600, color: T.accent, marginBottom: 6, letterSpacing: "0.02em" }}>让 AI 开口就是营销顾问</div>
            <div style={{ fontSize: 13, color: T.textDim }}>9 本营销必读经典提炼 · 可执行框架</div>
          </div>

          {/* 核心：一键安装 */}
          <div style={{ background: T.accent + "12", borderRadius: 16, padding: "24px 20px", border: `1px solid ${T.accent}30`, marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <code style={{ flex: 1, minWidth: 200, padding: "14px 18px", background: T.codeBg, borderRadius: 10, border: `1px solid ${T.border}`, fontSize: 14, fontFamily: "ui-monospace,monospace", color: T.accent }}>{INSTALL_CMD}</code>
              <button onClick={() => copyPrompt(INSTALL_CMD)} style={{ padding: "12px 20px", background: copiedIdx === INSTALL_CMD ? "#22c55e" : T.accent, color: "#000", borderRadius: 10, fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer", flexShrink: 0 }}>{copiedIdx === INSTALL_CMD ? "已复制" : "复制"}</button>
            </div>
          </div>
          ...
          <div style={{ background: T.card, borderRadius: 12, padding: "18px 20px", border: `1px solid ${T.border}`, marginBottom: 24 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                ...
                <div><span style={{ fontWeight: 600, color: T.text }}>9 本营销必读经典提炼</span> — 可执行框架</div>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: T.accent, flexShrink: 0, marginTop: 6 }} />
                <div><span style={{ fontWeight: 600, color: T.text }}>4 支柱全链路</span> — 定战略→做表达→促转化→保留存</div>
              </div>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: T.accent, flexShrink: 0, marginTop: 6 }} />
                <div><span style={{ fontWeight: 600, color: T.text }}>说任务即匹配</span> — 定位、文案、爆款、转化、留存全覆盖</div>
              </div>
            </div>
            <div onClick={() => setView("browse")} style={{ fontSize: 12, color: T.accent, cursor: "pointer", marginTop: 14, fontWeight: 600 }}>去浏览查看详情 →</div>
          </div>
          ...
          <div style={{ position: "relative", paddingLeft: 0 }}>
            <div style={{ position: "absolute", left: 27, top: 28, bottom: 28, width: 2, background: `linear-gradient(180deg,${T.accent} 0%,#e67e22 50%,#e74c3c 100%)`, borderRadius: 1 }} />
            {[
              { step: 1, title: "让 Agent 学习", examples: ["你去学一下营销方法论"], result: "升级龙虾大脑", color: T.accent },
              ...
              <div key={i} style={{ position: "relative", display: "flex", gap: 14, padding: "16px 18px 16px 56px", marginBottom: i < 2 ? 12 : 0, background: T.card, borderRadius: 12, border: `1px solid ${T.border}`, borderLeft: `4px solid ${m.color}`, transition: "all .15s" }}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 6 }}>
                    {m.examples.map((ex, j) => (
                      <code key={j} style={{ padding: "4px 10px", background: T.codeBg, borderRadius: 6, border: `1px solid ${T.border}`, fontSize: 11, fontFamily: "ui-monospace,monospace", color: T.accent }}>「{ex}」</code>
                    ))}
                  </div>
        ...
        {view === "diagnose" && !scene && <>
          ...
          <div style={{ color: T.textDim, fontSize: 12, marginBottom: 14 }}>按金字塔 4 大支柱选择，快速找到对应方法论</div>
          {/* 顶部四步快速导航 */}
          <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>
            {PYRAMID.map((p, pi) => {
        ...
        {view === "diagnose" && scene && <>
          ...
          <div style={{ fontSize: 11, color: T.accent, fontWeight: 600, marginBottom: 6 }}>首选方法论</div>
          ...
          <div style={{ marginTop: 16, padding: "14px 16px", background: T.accent + "12", borderRadius: 10, border: `1px solid ${T.accent}30` }}>
            <div style={{ fontSize: 11, color: T.accent, fontWeight: 600, marginBottom: 3 }}>为什么推荐？</div>
        ...
        {view === "quiz" && !done && (() => { const fw = FW[order.current[qi]]; return <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
            <div><div style={{ fontSize: 17, fontWeight: 800, color: T.text }}>营销方法论测试</div><div style={{ fontSize: 11, color: T.textDim }}>9道题 · 选完可看解析</div></div>
            <div style={{ textAlign: "right" }}><div style={{ fontSize: 18, fontWeight: 800, color: T.text }}>{score}<span style={{ fontSize: 12, color: T.textDim }}>/9</span></div><div style={{ fontSize: 11, color: T.textDim }}>当前得分</div></div>
          </div>
          ...
          {qa !== null && <button onClick={() => {
            ...
            style={{ marginTop: 16, padding: "11px 0", borderRadius: 10, background: T.accent, color: "#000", fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer", width: "100%" }}>{qi < 8 ? "下一题 →" : "查看结果"}</button>}
        </>; })()}

        {view === "quiz" && done && <div style={{ textAlign: "center", paddingTop: 20 }}>
          <div style={{ fontSize: 28, fontWeight: 400, fontFamily: "'DM Serif Display', Georgia, serif", color: T.text, marginBottom: 6 }}>{score >= 8 ? "营销方法论大师" : score >= 6 ? "进阶营销人" : score >= 4 ? "营销学徒" : "方法论新手"}</div>
          <div style={{ fontSize: 32, fontWeight: 900, color: T.text }}>{score}<span style={{ fontSize: 16, color: T.textDim }}>/9</span></div>
          <div style={{ fontSize: 15, fontWeight: 700, margin: "6px 0", color: score >= 8 ? "#22c55e" : score >= 6 ? T.accent : "#e67e22" }}>{score >= 8 ? "已掌握精髓" : score >= 6 ? "基础扎实" : score >= 4 ? "继续加油" : "建议多浏览"}</div>
          <div style={{ color: T.textDim, fontSize: 12, marginBottom: 20 }}>{score >= 8 ? "下一步实战运用。" : score >= 6 ? "复习答错的理论。" : "建议从浏览模式开始了解。"}</div>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => setView("browse")} style={{ padding: "10px 24px", borderRadius: 10, background: T.accent, color: "#000", fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer" }}>去学习</button>
          </div>
        </div>}
      </div>
