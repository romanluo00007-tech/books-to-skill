...
export default function App() {
  ...
  const INSTALL_CMD = "clawhub install marketing-playbook";
  if (!order.current) order.current = [...Array(9).keys()].sort(() => Math.random() - 0.5);
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"28px 20px 60px"}}>
        ...
        <div onClick={() => { setView("home"); setScene(null); }} style={{cursor:"pointer",marginBottom:20}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
            ...
            <span style={{...s.tag,background:"#f0c04015",color:"#f0c040",border:"1px solid #f0c04030"}}>营销方法论</span>
          </div>
          <div style={{fontSize:12,color:"#555"}}>9 个经典框架 · 9 本畅销书 · 让 AI 拥有顾问级营销能力</div>
        </div>

        {/* Nav - 标签式导航 */}
        <div style={{display:"flex",gap:0,borderBottom:"1px solid #222",marginBottom:24}}>
          {[["首页","home"],["浏览","browse"],["诊断","diagnose"],["测试","quiz"]].map(([lb,v]) => (
            <div key={v} onClick={() => { setView(v); setScene(null); setQi(0); setScore(0); setQa(null); setDone(false); setWrongFwIds([]); order.current=[...Array(9).keys()].sort(()=>Math.random()-0.5); }}
              style={{
                padding:"10px 16px",cursor:"pointer",fontSize:13,fontWeight:view===v?700:500,color:view===v?"#e5e5e5":"#666",
                borderBottom:view===v?"2px solid #f0c040":"2px solid transparent",
        ...
        {view === "home" && <>
          ...
          <div style={{textAlign:"center",padding:"32px 0 36px",marginBottom:8}}>
            <div style={{fontSize:32,fontWeight:800,color:"#e5e5e5",letterSpacing:"-0.02em",marginBottom:14}}>营销知识库 Skill</div>
            <div style={{fontSize:17,fontWeight:700,color:"#f0c040",marginBottom:6,letterSpacing:"0.02em"}}>让 AI 开口就是营销顾问</div>
            <div style={{fontSize:13,color:"#666"}}>9 本营销必读经典提炼 · 可执行框架</div>
          </div>

          {/* 核心：一键安装 */}
          <div style={{background:"linear-gradient(135deg,#f0c04012 0%,#e67e2215 100%)",borderRadius:16,padding:"24px 20px",border:"1px solid #f0c04030",marginBottom:28}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              <code style={{flex:1,minWidth:200,padding:"14px 18px",background:"#0c0c14",borderRadius:10,border:"1px solid #333",fontSize:14,fontFamily:"ui-monospace,monospace",color:"#f0c040"}}>{INSTALL_CMD}</code>
              <button onClick={() => copyPrompt(INSTALL_CMD)} style={{padding:"12px 20px",background:copiedIdx===INSTALL_CMD?"#22c55e":"#f0c040",color:"#000",borderRadius:10,fontSize:13,fontWeight:700,border:"none",cursor:"pointer",flexShrink:0}}>{copiedIdx===INSTALL_CMD?"已复制":"复制"}</button>
            </div>
          </div>
          ...
          <div style={{background:"#16161e",borderRadius:12,padding:"18px 20px",border:"1px solid #222",marginBottom:24}}>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                <span style={{width:6,height:6,borderRadius:"50%",background:"#f0c040",flexShrink:0,marginTop:6}} />
                <div><span style={{fontWeight:600,color:"#e5e5e5"}}>9 本营销必读经典提炼</span> — 可执行框架</div>
              </div>
              <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                <span style={{width:6,height:6,borderRadius:"50%",background:"#f0c040",flexShrink:0,marginTop:6}} />
                <div><span style={{fontWeight:600,color:"#e5e5e5"}}>4 支柱全链路</span> — 定战略→做表达→促转化→保留存</div>
              </div>
              <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                <span style={{width:6,height:6,borderRadius:"50%",background:"#f0c040",flexShrink:0,marginTop:6}} />
                <div><span style={{fontWeight:600,color:"#e5e5e5"}}>说任务即匹配</span> — 定位、文案、爆款、转化、留存全覆盖</div>
              </div>
            </div>
            <div onClick={() => setView("browse")} style={{fontSize:12,color:"#f0c040",cursor:"pointer",marginTop:14,fontWeight:600}}>去浏览查看详情 →</div>
          </div>
          ...
          <div style={{position:"relative",paddingLeft:0}}>
            {/* 左侧流程线 */}
            <div style={{position:"absolute",left:27,top:28,bottom:28,width:2,background:"linear-gradient(180deg,#f0c040 0%,#e67e22 50%,#e74c3c 100%)",borderRadius:1}} />
            {[
              { step: 1, icon: "📖", title: "让 Agent 学习", examples: ["你去学一下营销方法论"], result: "升级龙虾大脑", color: "#f0c040" },
              { step: 2, icon: "💬", title: "在相关任务中自动调用", examples: ["帮我分析这条内容为什么火", "帮我写卖点", "帮我做定位"], result: "Agent 自动匹配框架", color: "#e67e22" },
              { step: 3, icon: "🎯", title: "指定具体框架进行分析", examples: ["用 STEPPS 帮我分析"], result: "精确调用某一框架", color: "#e74c3c" },
            ].map((m, i) => (
              <div key={i} style={{position:"relative",display:"flex",gap:14,padding:"16px 18px 16px 56px",marginBottom:i<2?12:0,background:"#16161e",borderRadius:12,border:"1px solid #222",borderLeft:`3px solid ${m.color}`,transition:"all .15s"}}
                ...
                <span style={{position:"absolute",left:18,top:"50%",transform:"translateY(-50%)",width:22,height:22,borderRadius:"50%",background:m.color,border:"3px solid #0c0c14",boxSizing:"border-box",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,color:"#000",zIndex:1}}>{m.step}</span>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:6}}>
                    {m.examples.map((ex, j) => (
                      <code key={j} style={{padding:"4px 10px",background:"#0c0c14",borderRadius:6,border:"1px solid #333",fontSize:11,fontFamily:"ui-monospace,monospace",color:"#f0c040"}}>「{ex}」</code>
                    ))}
                  </div>
        ...
        {view === "diagnose" && scene && <>
          ...
          <div style={{fontSize:11,color:"#f0c040",fontWeight:600,marginBottom:6}}>🎯 首选方法论</div>
          ...
          <div style={{marginTop:16,padding:"14px 16px",background:"#f0c04008",borderRadius:10,border:"1px solid #f0c04018"}}>
            <div style={{fontSize:11,color:"#f0c040",fontWeight:600,marginBottom:3}}>💡 为什么推荐？</div>
        ...
        {view === "quiz" && !done && (() => { const fw = FW[order.current[qi]]; return <>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
            <div><div style={{fontSize:17,fontWeight:800}}>营销方法论测试</div><div style={{fontSize:11,color:"#666"}}>9道题 · 选完可看解析</div></div>
            <div style={{textAlign:"right"}}><div style={{fontSize:18,fontWeight:800}}>{score}<span style={{fontSize:12,color:"#555"}}>/9</span></div><div style={{fontSize:11,color:"#666"}}>当前得分</div></div>
          </div>
          ...
          {qa !== null && <button onClick={() => {
            ...
            if (qi<8) { setQi(q=>q+1); setQa(null); } else setDone(true);
          }}
            style={{marginTop:16,padding:"11px 0",borderRadius:10,background:"#f0c040",color:"#000",fontSize:13,fontWeight:700,border:"none",cursor:"pointer",width:"100%"}}>{qi<8?"下一题 →":"查看结果"}</button>}
        </>; })()}

        {view === "quiz" && done && <div style={{textAlign:"center",paddingTop:20}}>
          <div style={{fontSize:42,marginBottom:6}}>{score>=8?"🏆":score>=6?"🎯":score>=4?"📖":"🌱"}</div>
          <div style={{fontSize:32,fontWeight:900}}>{score}<span style={{fontSize:16,color:"#555"}}>/9</span></div>
          <div style={{fontSize:15,fontWeight:700,margin:"6px 0",color:score>=8?"#4ade80":score>=6?"#f0c040":"#e67e22"}}>{score>=8?"营销方法论大师":score>=6?"进阶营销人":score>=4?"营销学徒":"方法论新手"}</div>
          <div style={{color:"#666",fontSize:12,marginBottom:20}}>{score>=8?"已掌握精髓，下一步实战运用。":score>=6?"基础扎实！复习答错的理论。":"建议从浏览模式开始了解。"}</div>
          ...
          <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
            <button onClick={() => { setQi(0);setScore(0);setQa(null);setDone(false);setWrongFwIds([]);order.current=[...Array(9).keys()].sort(()=>Math.random()-0.5); }}
            ...
            <button onClick={() => setView("browse")} style={{padding:"10px 24px",borderRadius:10,background:"#f0c040",color:"#000",fontSize:13,fontWeight:700,border:"none",cursor:"pointer"}}>去学习</button>
          </div>
        </div>}
      </div>
